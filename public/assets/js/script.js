(function () {
	...
	allSelect.forEach(function (select, index) {
		...
		mainContainer.setAttribute("data-select-head", "custom-select" + index + "");
		...
		var SelectOptionsList = document.querySelector(".custom-select-options[data-select-head=" + "custom-select" + index + "]");



		function createHead(headElem) {
      var DefaultSelectedValue = selectElm.querySelector("[selected]") || selectElm.querySelector('option');
      Object.assign(headElem,{
		...
		function appendingOptions(list) {
			...
			for (var x = 0; x < list.length; x++) {
				if (list[x].tagName == "OPTION") {
					code = "<li data-value=" + list[x].value + ">" + list[x].textContent + "</li>";
				} else {
					var head = "<li class='label'><h3>" + list[x].getAttribute('label') + "</li></h3>"
					var lists = ""
					var optgroupChildren = Array.from(list[x].children);
					for (var i = 0; i < optgroupChildren.length; i++) {
						lists += "<li data-value=" + optgroupChildren[i].value + ">" + optgroupChildren[i].textContent + "</li>";
					}
					code = head + lists
				}
				// CONDITION TO CHECK IF CHECKBOX HAS IMAGES
				if (select.classList.contains("icon-imgs") && list[x].dataset.imgUrl) {
					code = "<li data-value=" + list[x].value + "> <div class='custom-select-img-container'><img src='" + list[x].dataset.imgUrl + "'></div>" + list[x].textContent + "</li>";
				}
				liContainer += code;
			}
		...
		activeHead.addEventListener("click", function (e) {
			...
			var thisList = document.querySelector(".custom-select-options[data-select-head=" + thisSelectIndex + "]");
			if (thisList.classList.contains("is-active")) {
				...
				var allLists = Array.from(document.querySelectorAll(".custom-select-options"));
		...
		var searchBoxes = Array.from(SelectOptionsList.querySelectorAll(".search-list"));
	...
	var allOptions = Array.from(document.querySelectorAll(".custom-select-options"));
	// KEYBOARD EVENTS
	document.addEventListener('keydown', function (event) {
		var activeOption = document.querySelector(".custom-select-options.is-active ul") || "";
		var activeUl = document.querySelector(".custom-select-options.is-active") || "";
		if (activeOption) {
			var selcted = activeUl.getAttribute('data-select-head')
			var selectedLi = activeOption.querySelector('li[data-value].is-highlighted');
			var SelectHead = document.querySelector("[data-select-index=" + selcted + "]");
			...
			if (event.keyCode == 40) {
				if (selectedLi && selectedLi.nextSibling !== null) {
					...
					} else if (selectedLi.nextSibling.nextSibling.getAttribute('data-value') !== null) {
						selectedLi.classList.remove('is-highlighted');
						selectedLi.nextSibling.nextSibling.classList.add('is-highlighted');
					}
				} else {
					selectedLi.classList.remove('is-highlighted')
					activeOption.querySelector('li[data-value]').classList.add('is-highlighted');
				}
				...
				if (selectedLi.previousSibling !== null) {
					...
					} else if (selectedLi.previousSibling.previousSibling !== null && selectedLi.previousSibling.previousSibling.getAttribute('data-value') !== null) {
						selectedLi.classList.remove('is-highlighted');
						selectedLi.previousSibling.previousSibling.classList.add('is-highlighted');
					}
				...
				var selectElement = document.querySelector(".custom-select-options.is-active li.is-highlighted")
